 ...
 */
export default function qbs(options) {
	...
	_.extend(dash, {
		...
		init: function (options) {
			...
			console.log("qbs(): %o -> %o %o", dash.options, $el, dash)
		...
		qb: function (qb_id) {
			if (!qb_id) throw "qbs:qb:anonymous"
			var dash_qb = dash._qbs[qb_id]
			if (!dash_qb) throw "qbs:qb:missing#" + qb_id
			return dash_qb
		},
		register: {
			qb: function (qb_id, dash_conf) {
				if (!qb_id) throw "qbs:register:qb:missing:id"
				if (!dash_conf) throw "qbs:register:qb:missing:config"
